...
export const convertImageBlob = async (blob: Blob, targetExt: string): Promise<Blob> => {
  ...
  const targetMime = mimeTypes[targetExt.toLowerCase()];
  if (!targetMime || blob.type === targetMime) {
    return blob;
  }

  // Canvas conversion for standard formats
  if (["png", "jpg", "jpeg", "webp"].includes(targetExt.toLowerCase())) {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          ...
          targetMime,
          targetExt.toLowerCase() === "jpg" || targetExt.toLowerCase() === "jpeg" ? 0.92 : undefined
        );
      };
  ...
  // Fallback for gif or other unsupported conversions
  return blob;
};